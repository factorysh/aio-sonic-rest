...
async def query(request):
    r = await request.app['sonic'].query(request.app['site'],
...
async def suggest(request):
    ...
    r = await request.app['sonic'].suggest(request.app['site'],
                                           request.query.get('bucket', 'body'),
                                           s, 5)
    return web.json_response(r)


async def Search(app, host='127.0.0.1', port=1491,
                 password=None, site='site', store='./data/kv'):
    app['sonic'] = Client(host=host, port=port,
                          password=password,
                          max_connections=100)
